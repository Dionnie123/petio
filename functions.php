...
require_once(get_template_directory() . '/customizations/custom-css.php');
require_once(get_template_directory() . '/customizations/category-filter.php');

function petio_custom_css() {
	...
	if (!is_admin()) {
		...
		wp_add_inline_style( 'petio-style-template', implode($custom_css) );
	}
}
add_action('wp_enqueue_scripts', 'petio_custom_css' );
function petio_custom_js() {
	if (!is_admin()) {
		wp_enqueue_script( 'petio-script', get_template_directory_uri() . '/js/functions.js', array( 'jquery'), null, true );
		wp_localize_script( 'petio-script', 'petio_ajax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );
		...
		wp_add_inline_script( 'petio-script', $custom_js);
	}
}
add_action('wp_enqueue_scripts', 'petio_custom_js' );
